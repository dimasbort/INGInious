...
class CourseDangerZonePage(INGIniousAdminPage):
    ...
    def wipe_course(self, courseid):
        ...
        self.database.audiences.delete_one({"courseid": courseid})
        self.database.groups.delete_one({"courseid": courseid})
        self.database.user_tasks.delete_one({"courseid": courseid})
        self.database.submissions.delete_one({"courseid": courseid})
    ...
    def restore_course(self, courseid, backup):
        ...
        with zipfile.ZipFile(filepath, "r") as zipf:
            ...
            if len(audiences) > 0:
                self.database.audiences.insert_one(audiences)
            ...
            if len(user_tasks) > 0:
                self.database.user_tasks.insert_one(user_tasks)
            ...
            if len(submissions) > 0:
                self.database.submissions.insert_one(submissions)
